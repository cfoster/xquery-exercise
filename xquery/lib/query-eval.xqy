...
as cts:query?
{
  ...
  else qe:field-to-word-query($qnames, $list)
};

declare private function qe:field-to-word-query(
  $qnames as xs:QName+,
  $list as element()*) as cts:query? {
  if($qnames = xs:QName("type")) then
    cts:collection-query("data-" || $list)
  else
    cts:element-query($qnames, $list)
};
